...
def test_pipeline_runs_and_processes(dummy_loader, dummy_processor):
    pipeline = DataPipeline(loader=dummy_loader, processors=[dummy_processor])
...
def test_pipeline_multiple_processors(dummy_loader):
    ...
    pipeline = DataPipeline(
        loader=dummy_loader, processors=[AddOneProcessor(), AddOneProcessor()]
    )
    processed = pipeline.run("dummy_path.csv")
    assert all(processed.data["a"] == [3, 4])


def test_pipeline_empty_processors(dummy_loader):
    pipeline = DataPipeline(loader=dummy_loader, processors=[])
    processed = pipeline.run("dummy_path.csv")
    assert isinstance(processed, ProcessedData)
