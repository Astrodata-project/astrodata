...
from astrodata.ml.model_selection._utils import cross_validation_grid_search, single_split_grid_search
...
class HyperOptSelector(BaseMlModelSelector):
    ...
    def _objective(self, params, X, y, X_val=None, y_val=None):
        # hyperopt passes numpy floats, so cast where needed
        params = {k: (int(v) if isinstance(v, np.generic) and isinstance(v.item(), int) else v) for k, v in params.items()}
        model = params.pop("model")
        
        if not isinstance(model, BaseMlModel):
            raise f"{model} is not a BaseMlModel instance"
    ...
    def fit(
        ...
        # hyperopt may return numpy types
        best_params = {k: (int(v) if isinstance(v, np.generic) and isinstance(v.item(), int) else v) for k, v in best_params.items()}
        if self.use_cv:
            X_full, y_full = X, y
        else:
        ...
        self._best_model = self._best_params.pop('model').clone()
        ...
        self._best_metrics = self._best_model.get_metrics(X=X_test, y=y_test, metrics=self.metrics)
...
def _map_hyperopt_best_params(best_params, search_space):
    """
